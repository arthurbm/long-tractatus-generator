...
export async function POST(req: Request) {
  const { prompt, returnType, modelValue } = await req.json() as { 
    ...
    modelValue: 'gpt-4o-mini' | 'gemini-flash-1.5'
  };
...
`;
  ...
  const jsonSpecificInstructions = `
**Task and Intent:**  
**Task:**  
...
- Respond only with the final structured output (JSON or text, one per section), without external commentary.
`;

  const system = `${baseSystem}${returnType === 'json' ? jsonSpecificInstructions : textSpecificInstructions}${commonInstructions}`;
